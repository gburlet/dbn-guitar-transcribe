...
Program::Program (int n, double* x, double* lb, double* ub, int* btype, 
		  int m, int maxiter, int maxFuncEval, double factr, double pgtol) {
  this->n       = n;
  this->x       = x;
  this->lb      = lb;
  this->ub      = ub;
...
Program::Program (int n, int m, int maxiter, int maxFuncEval, double factr, double pgtol) {
  this->n       = n;
  this->x       = new double[n];
...
SolverExitStatus Program::runSolver() {
  status = success;  // The return value.
...
//     g[i] = 0.0;
  ...
  while (true) {
    ...
    if (strIsEqualToCStr(task,"FG")) {
      ...
      f = computeObjAndGradient (n, x, g);
      num_func_eval ++;
      if (num_func_eval == maxFuncEval) {
          callLBFGS("STOP");
          status = maxFuncEvalReached;
          break;
      }
      ...
      if (num_iter == maxiter) {
        ...
        if (strIsEqualToCStr(task,"CONVERGENCE: NORM OF PROJECTED GRADIENT"))
            status = smallPG;
        else if (strIsEqualToCStr(task,"CONVERGENCE: REL_REDUCTION_OF_F"))
            status = smallRelChange;
        break;
    }
